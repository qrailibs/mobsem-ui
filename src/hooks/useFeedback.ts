...
export function usePressFeedback<T extends HTMLElement>(
	...
	options?: { pressedAttr?: string; bounceAttr?: string }
) {
	...
	return useMemo(() => {
		const onPointerDown: React.PointerEventHandler<T> = (event) => {
			(event.currentTarget as HTMLElement).removeAttribute(bounceAttr);
			(event.currentTarget as HTMLElement).setAttribute(pressedAttr, "true");
		...
		const onPointerUp: React.PointerEventHandler<T> = (event) => {
			releaseWithBounce(event.currentTarget as HTMLElement);
	...
	}, [userHandlers, pressedAttr, bounceAttr]);
}
